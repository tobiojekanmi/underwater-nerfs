...
from models.seathrunerf.seathrunerf_field import SeathruNerfField, MediumFieldHeadNames
from models.seathrunerf.seathrunerf_renderers import SeathruNeRFRGBRenderer
from models.seathrunerf.seathrunerf_losses import reconstruction_loss, accumulation_loss
from nerfstudio.utils.rich_utils import CONSOLE


@dataclass
class SeathruNerfModelConfig(ModelConfig):
    """SeathruNerf Model Config"""

    _target: Type = field(default_factory=lambda: SeathruNerfModel)
    ...
    """Whether to use the viewing direction for object RGB."""
    ...
    accumulation_loss_mult: float = 0.0001
    """Accumulation loss multiplier."""
    accumulation_loss_beta_prior: float = 100.0
    """Beta prior for the accumulation loss."""


class SeathruNerfModel(Model):
    """Nerfacto model
    ...
    def populate_modules(self):
        ...
        self.field = SeathruNerfField(
            ...
            use_viewing_direction_for_object_rgb=self.config.use_viewing_direction_for_object_rgb,
        )
        ...
        # renderers
        self.renderer_rgb = SeathruNeRFRGBRenderer()
        self.renderer_accumulation = AccumulationRenderer()
        self.renderer_depth = DepthRenderer(method="median")
        self.renderer_expected_depth = DepthRenderer(method="expected")

        # losses
        self.rgb_loss = reconstruction_loss
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        outs = self.renderer_rgb(
            object_rgbs=field_outputs[FieldHeadNames.RGB],
            medium_rgbs=field_outputs[MediumFieldHeadNames.MEDIUM_RGB],  # type: ignore
            direct_coeffs=field_outputs[MediumFieldHeadNames.DIRECT_COEFFS],  # type: ignore
            backscatter_coeffs=field_outputs[MediumFieldHeadNames.BACKSCATTER_COEFFS],  # type: ignore
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        if self.training:
            ...
            loss_dict["transmittance_loss"] = (
                ...
                * accumulation_loss(
                    transmittance_object=outputs["object_transmittance"],
                    beta=self.config.accumulation_loss_beta_prior,
                )
            )
    ...
    def get_image_metrics_and_images(
        ...
        if outputs["restored"] is not None and outputs["direct"] is not None:
            images_dict["gt_direct_restored"] = torch.cat(
                [gt_rgb, outputs["direct"], outputs["restored"]], dim=1
            )

        if outputs["backscatter"] is not None:
            images_dict["backscatter"] = torch.cat([outputs["backscatter"]], dim=1)
    ...
    def get_rgba_image(
        self, outputs: Dict[str, torch.Tensor], output_name: str = "rgb"
    ) -> torch.Tensor:
